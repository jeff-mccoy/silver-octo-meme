...
const REFLECTION_SESSION = "__reflection__";

let matrixClient = null;
let targetRoomId = null;
let timer = null;

const REFLECTION_PROMPT = `[Daily reflection — ${new Date().toISOString()}]

It's time for your daily self-reflection. This is your chance to maintain your mind.

## Step 1: Review memories
...
export function startReflection(client, roomId) {
  ...
  console.log(
    `[reflection] starting (interval: ${INTERVAL_MS / 1000 / 3600}h)`,
  );
...
async function runReflection() {
  try {
    ...
    const response = await runPrompt(REFLECTION_SESSION, REFLECTION_PROMPT);
    ...
    if (matrixClient && targetRoomId) {
      // Only post if the summary is substantive (not just "pruned 1 memory")
